#!/usr/bin/env python3
"""Match each enriched concept to the most relevant SECTION of every bible it
cites, and record the pointers as `sources: {bible: anchor}` frontmatter.

Uses the bible-corpus semantic index to find the closest section, resolves the
...
CORPUS = os.path.join(WIKI, ".bibles-text")
...
def nearest_anchor(md_text, line, anchors_b):
    ...
    non-null anchor. Well-anchored bibles fall back to their h2 when the h3 is
    unanchored; re-sectioned coarse bibles resolve to the finer h3."""
...
def _query(bible, q):
    out = subprocess.run(
        [sys.executable, "-B", SEARCH, "--cwd", CORPUS, "--in", f"{bible}.md",
         "--json", q, "--top", "5"],
...
def match_page(page_path, anchors):
    ...
    bm = re.search(r"bibles:\s*\[(.*?)\]", front, re.S)
    bibles = re.findall(r"[A-Za-z0-9][\w-]+", bm.group(1)) if bm else []
    ...
    for b in bibles:
        hit = _query(b, query)
        if not hit:
            print(f"  [match] {os.path.basename(page_path)}: no corpus hit for {b} (→ whole-bible fallback)")
            continue
        ...
        if anc:
            ...
            print(f"  [match] {os.path.basename(page_path)}: hit for {b} had no anchored heading (→ whole-bible fallback)")
    return src
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Write concept→bible-section pointers.")
