...
- **Version control is scoped to the wiki ONLY:** `git init` inside `/Users/noahraford/magic/wiki`, NEVER at `magic/` scope — that would place the read-only sources inside a repo whose rollback commands could write or delete under them.
...
Write an empty `/Users/noahraford/magic/wiki/log.md` (a single `# research-wiki log` header line is fine). Then initialise the wiki's own repo:
